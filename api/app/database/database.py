...
# "mongodb+srv://root:root@cluster0-cg3ab.mongodb.net/test?retryWrites=true&w=majority"


class Database:
    def __init__(self):
        self._client = pymongo.MongoClient(DATABASE_URI)
        self.admin_db = self._client["admins"]['admins']
        self.doc_db = self._client["documents"]
    ...
    def get_user_stored_password(self, username):
        data = self.admin_db.find_one({'username' : username})
        if not data:
            return ''   
